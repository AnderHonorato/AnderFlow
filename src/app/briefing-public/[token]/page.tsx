...
import {
  ...
  X, Image as IconeImagem, File, Upload,
} from 'lucide-react'
...
const DRAFT_KEY = 'briefing_public_draft_id'
...
export default function BriefingPublicPage() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center min-h-screen"><svg className="animate-spin h-6 w-6 text-[var(--text-3)]" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M8 2a6 6 0 016 6"/></svg></div>}>
      <BriefingPublicContent />
    </Suspense>
  )
}

function BriefingPublicContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/briefing/validate?token=${encodeURIComponent(token)}`)
      .then(r => r.json())
      .then(data => {
  ...
  const saveDraft = useCallback(async () => {
    if (!clientId || !selectedCategory || !mountedRef.current) return
    setSaving(true)
    const res = await fetch('/api/briefing', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'save', userId: clientId, categoryId: selectedCategory, draftId, currentStage: stageIndex, currentStep: 0, answers }),
    })
    ...
    setSaving(false)
  }, [clientId, selectedCategory, draftId, stageIndex, answers])
    ...
    if (stageIndex < totalStages - 1) {
      ...
      try {
        const res = await fetch('/api/briefing', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action: 'submit', userId: clientId, categoryId: selectedCategory, draftId, answers }),
        })
        const json = await res.json()
        ...
        return (
          <div className="space-y-2">
            <div
              ...
              onClick={() => document.getElementById(`file-input-pub-${q.id}`)?.click()}
            >
      ...
      <div className="min-h-screen bg-[var(--bg)] flex items-center justify-center">
        <Card className="max-w-sm mx-4">
          <CardContent className="p-8 text-center space-y-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[var(--destructive-subtle)] mx-auto">
              <X className="h-6 w-6 text-[var(--destructive)]" />
            </div>
            <h2 className="text-base font-medium text-[var(--text)]">Link inválido ou expirado</h2>
            <p className="text-xs text-[var(--text-muted)]">O link de briefing não é mais válido. Solicite um novo link ao administrador.</p>
          </CardContent>
        </Card>
      </div>
    )
        ...
        {step === 'summary' && (
          <Card>
            <CardContent className="p-6 space-y-5 text-center">
              ...
              {projectId && (
                <p className="text-2xs text-[var(--text-muted)]">Protocolo #{projectId.slice(-8).toUpperCase()}</p>
              )}
            </CardContent>
          </Card>
        )}
